...
const MENU_ITEMS: {
  label: string;
  icon: string;
  screen: keyof TopTabParamList;
}[] = [
  { label: "Lista", icon: "list", screen: "Lista" },
...
function TopTabBarWithSearch(props: MaterialTopTabBarProps) {
  ...
  function navigateTo(screen: keyof TopTabParamList) {
    closeDrawer(() => props.navigation.navigate(screen));
  }
          ...
          >
            ...
            {MENU_ITEMS.map((item) => (
              <Pressable
                ...
                style={({ pressed }) => [
                  styles.drawerItem,
                  activeRoute.name === item.screen && styles.drawerItemActive,
                  pressed && styles.drawerItemPressed,
                ]}
                onPress={() => navigateTo(item.screen)}
              >
                <Ionicons
                  name={item.icon as "list" | "map"}
                  size={20}
                  color={activeRoute.name === item.screen ? "#fff" : "#171717"}
                />
                <Text
                  style={[
                    styles.drawerItemText,
                    activeRoute.name === item.screen &&
